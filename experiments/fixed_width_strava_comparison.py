"""Coverage comparison: shelf vs glacier in fixed-width mode on strava data, N=10 and N=100, 100 seeds each."""
...
for n in NS:
    for algo in ALGORITHMS:
        ...
        inner_w = WIDTH - 2 * EDGE_GAP
        for seed in SEEDS:
            rng = random.Random(seed)
            boxes = [b for b in rng.sample(all_boxes, n) if b.width <= inner_w]
            if not boxes:
...
def render_panel(n: int, panel_y: int) -> list[str]:
    ...
    lines.append(
        ...
        f'strava sample  n={n}  |  {len(SEEDS)} seeds  |  width={WIDTH}  gap={GAP}</text>'
    )
...
hist_out = Path(__file__).parent / "fixed_width_strava_histogram.svg"
hist_out.write_text("\n".join(hist_lines))
print(f"\nSaved → {hist_out}")
...
LABEL_H = 24
COLS = ["shelf worst", "shelf median", "shelf best",
...
for row, n in enumerate(NS):
    ...
    for col, (algo, which) in enumerate(cells):
        seed, result = notables[(algo, n)][which]
        cov = result.coverage
...
# Thicker divider between shelf and glacier columns
bw_lines.append(
...
bw_out = Path(__file__).parent / "fixed_width_strava_layouts.svg"
bw_out.write_text("\n".join(bw_lines))
