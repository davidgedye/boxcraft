"""Coverage comparison: shelf vs glacier in fixed-width mode, N=10 and N=100, 100 seeds each."""
...
GAP = 5
EDGE_GAP = 5
WIDTH = 500   # fixed container width
...
for n in NS:
    for algo in ALGORITHMS:
        ...
        # Median: re-pack the seed closest to the median coverage
...
def render_panel(n: int, panel_y: int) -> list[str]:
    ...
    lines.append(
        ...
        f'uniform U[10,200]²  n={n}  |  {len(SEEDS)} seeds  |  width={WIDTH}  gap={GAP}</text>'
    )
...
hist_out = Path(__file__).parent / "fixed_width_histogram.svg"
hist_out.write_text("\n".join(hist_lines))
print(f"\nSaved → {hist_out}")
...
bw_out = Path(__file__).parent / "fixed_width_layouts.svg"
bw_out.write_text("\n".join(bw_lines))
