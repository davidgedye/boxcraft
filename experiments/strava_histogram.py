"""Coverage histogram: shelf vs glacier on strava data, N=10 and N=100, 100 samples each."""
...
for n in NS:
    for algo in ALGORITHMS:
        ...
        median_seed, median_result = -1, None
        ...
        # Find the sample closest to the median
        ...
        print(f"{algo:8s} n={n:4d}  mean={mean:.1f}%  range=[{lo:.1f}%, {hi:.1f}%]  "
              f"best seed={best_seed}  worst seed={worst_seed}  median seed={median_seed}")
...
def render_panel(n: int, panel_y: int) -> list[str]:
    ...
    lines.append(
        ...
        f'strava sample  n={n}  |  {len(SEEDS)} seeds  |  gap={GAP}  ar={ASPECT:.3f}</text>'
    )
...
hist_out = Path(__file__).parent / "strava_histogram.svg"
hist_out.write_text("\n".join(hist_lines))
print(f"\nSaved → {hist_out}")
...
bw_out = Path(__file__).parent / "strava_best_median_worst.svg"
bw_out.write_text("\n".join(bw_lines))
